...
let nameCollection = ["zade","Jarom","Beau", "Lincoln", "Grace", "Alec", "Josh"]

let sortedCollection = nameCollection.sorted(by: forwards(name1:name2:))
...
let sortedCollectionByClosure = nameCollection.sorted { someName, someOtherName in
    return someName > someOtherName
}
...
let sortedCollectionByClosureSimplified = nameCollection.sorted { $0 > $1 }
