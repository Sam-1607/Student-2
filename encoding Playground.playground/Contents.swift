...
struct Note: Codable {
    ...
    let text: String
    let timestamp: Date
    //property here is custom object and doesn't support Codable
}
...
let mypropertyListEncoder = PropertyListEncoder()
let encodedNotes = try? mypropertyListEncoder.encode(notes)
